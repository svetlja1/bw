...
__device__ __constant__ int r[24] = {
...
__inline__ __device__ void keccak256(uint64_t * __restrict__ state){
    uint64_t temp, C[5];
	int j;
#pragma unroll	
    for (int i = 0; i < 24; i++) {
...
__inline__ __device__ void keccak(const char * __restrict__ message, int message_len, unsigned char * __restrict__ output, int output_len){
    uint64_t state[25];    
    uint8_t temp[144];
    int rsize = 136;
    int rsize_byte = 17;
